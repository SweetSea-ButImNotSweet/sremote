...
| :--- | :--- | :--- | :--- |
| `instanceId` | `string \| HTMLElement` | `null` | Identifier of the media instance to query. If omitted, queries the most recently active instance. |
| `key` | `string` | `null` | Passkey authentication if domain lock is enabled. |

## Return Value
Returns an `SRemoteMediaState` object containing:
...
- `loop` (`boolean`): Whether media loop is enabled.
- `repeat` (`'off' | 'one' | 'all' | boolean`): Repeat mode of the playlist or player.
- `shuffle` (`boolean`): Whether shuffle playback is enabled.
- `quality` (`string | number`): Current quality or resolution level.
- `subtitle` (`string | null`): Active subtitle track ID or language code.
