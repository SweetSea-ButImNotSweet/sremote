...
- **Trường hợp `MISSING_MEDIA_SOURCE`**: Nếu website/iframe tạo thẻ `<video>` hoặc `<audio>` rỗng nhưng chưa nạp `src` (hoặc `MediaSource`) và `readyState = 0` (thường gặp khi trang web đợi người dùng click nút Play của họ trước), lệnh `play()` sẽ log cảnh báo và trả về `{ error: 'MISSING_MEDIA_SOURCE' }` để báo hiệu media chưa có dữ liệu phát.
