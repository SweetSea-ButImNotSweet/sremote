...
| Nhóm | Các hàm API |
| :--- | :--- |
| **Kết nối & Khởi tạo** | `hello(options?, target?)`<br>`assignId(iframeOrSelector, customId)` |
| **Giao tiếp & RPC** | `postWindowMessage(message, targetOrigin?, instanceId?, from?, key?)`<br>`onWindowMessage(handler, key?)`<br>`call(action, params?, instanceId?, key?)` |
| **CSS Iframe** | `setIframeCSS(css, instanceId?, key?)`<br>`getIframeCSS(instanceId?, key?)`<br>`removeIframeCSS(instanceId?, key?)` |
| **Điều khiển phát** | `play(instanceId?, key?)`<br>`pause(instanceId?, key?)`<br>`toggle(instanceId?, key?)`<br>`stop(instanceId?, key?)`<br>`playbackRate(rate, instanceId?, key?)` |
...
| **Hiển thị & DOM** | `pip(enable?, instanceId?, key?)`<br>`getIframe(instanceId, key?)` |
...
| **Bảo mật** | `lock()` |
| **Chẩn đoán & Test (Debug)** | `debug.scan()`<br>`debug.getState(instanceId?)`<br>`debug.dump(instanceId?)`<br>`debug.setSource(source, instanceId?)`<br>`debug.injectTestTone(freq?, dur?, instanceId?)`<br>`debug.injectSilentTrack(dur?, instanceId?)`<br>`debug.injectWhiteNoise(dur?, instanceId?)`<br>`debug.injectSampleVideo(instanceId?)`<br>`debug.simulateStall(instanceId?)`<br>`debug.restoreOriginal(instanceId?)` |
| **Xử lý & Tra cứu Lỗi** | [Xem Bảng Mã Lỗi & Khắc Phục Sự Cố](../../guides/vi/errors.md) |
