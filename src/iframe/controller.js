...
export function bindMediaSessionDefaults({ activeMediaGetter, mediaTypeGetter, resolveActiveMedia }) {
  ...
  const artworkSrc =
    getMeta(['meta[property="og:image"]', 'meta[name="twitter:image"]', 'link[rel="image_src"]']) ||
...
export function createMediaController({
  ...
  return async function executeControl(action, value, isPureGet = false) {
    ...
    const canHandleNorm =
      mockMediaSessionInstance._handlers.has(norm) ||
      (norm === 'toggle' && (mockMediaSessionInstance._handlers.has('play') || mockMediaSessionInstance._handlers.has('pause')));
