...
`;function g(){if(typeof navigator>`u`)return`chrome`;let e=navigator.userAgent.toLowerCase();return e.includes(`firefox`)?`firefox`:e.includes(`edg/`)?`edge`:e.includes(`opr/`)||e.includes(`opera/`)?`opera`:e.includes(`safari`)&&!e.includes(`chrome`)?`safari`:`chrome`}var _={chrome:{tampermonkey:`https://chromewebstore.google.com/detail/tampermonkey/dhdgffkkmingnoiobeogfiigjmhednnj`,violentmonkey:`https://chromewebstore.google.com/detail/violentmonkey/jinjaccalgkegednnccohejagnlnfdag`},firefox:{tampermonkey:`https://addons.mozilla.org/firefox/addon/tampermonkey/`,violentmonkey:`https://addons.mozilla.org/firefox/addon/violentmonkey/`},edge:{tampermonkey:`https://microsoftedge.microsoft.com/addons/detail/tampermonkey/iikmkjmpaadaobahmlepgglflondmnje`,violentmonkey:`https://microsoftedge.microsoft.com/addons/detail/violentmonkey/eeagobfjfgddacbcigncyclcoaebeent`},opera:{tampermonkey:`https://addons.opera.com/extensions/details/tampermonkey-beta/`},safari:{tampermonkey:`https://apps.apple.com/app/tampermonkey/id1482490089`}},v=null;function y(e={}){if(typeof document>`u`)return{host:null,close:()=>{}};v&&v.close();let{userscriptUrl:t=`https://raw.githubusercontent.com/SweetSea-ButImNotSweet/sremote/main/dist/sremote.user.js`,title:n=`Yêu cầu SRemote Userscript`,description:r=`Trang web cần SRemote Userscript để tương tác và điều khiển media trong iframe cross-origin.`,autoDetect:i=!0,onClose:a=null,onSuccess:o=null}=e,s=g(),c=document.createElement(`div`);c.id=`sremote-install-modal-host`;let l=c.attachShadow({mode:`closed`}),u=document.createElement(`style`);u.textContent=h;let d=document.createElement(`dialog`),f=document.createElement(`div`);f.className=`sv-box sv-install-box`;let p=!1,m=null,y=!1,b=()=>{if(!p){p=!0,typeof window<`u`&&window.removeEventListener(`sremote:ready`,F);try{d.close()}catch{}c.remove(),v?.host===c&&(v=null),a?.({success:y})}};v={host:c,close:b};let x=document.createElement(`div`);x.className=`sv-install-header`;let S=document.createElement(`div`);S.className=`sv-install-title`,S.textContent=n;let C=document.createElement(`button`);if(C.className=`sv-install-close-btn`,C.innerHTML=`&times;`,C.title=`Đóng`,C.addEventListener(`click`,e=>{e.stopPropagation(),b()}),x.append(S,C),f.append(x),r){let e=document.createElement(`div`);e.className=`sv-text`,e.textContent=r,f.append(e)}m=document.createElement(`div`),m.className=`sv-status-banner waiting`,m.innerHTML=`
    <div class="sv-status-spinner"></div>
    <span>Chờ nhận diện Userscript...</span>
  `,f.append(m);let w=document.createElement(`div`);w.className=`sv-steps`;let T=document.createElement(`div`);T.className=`sv-step`;let E=_[s]||_.chrome,D=``;E.tampermonkey&&(D+=`<a class="sv-ext-link sv-ext-recommended" href="${E.tampermonkey}" target="_blank" rel="noopener noreferrer">Tampermonkey (${s})</a>`),E.violentmonkey&&(D+=`<a class="sv-ext-link" href="${E.violentmonkey}" target="_blank" rel="noopener noreferrer">Violentmonkey</a>`),T.innerHTML=`
