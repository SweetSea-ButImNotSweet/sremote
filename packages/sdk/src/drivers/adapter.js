...
export class AdapterDriver {
   ...
   * @param {string} actionName - Action name (e.g. 'play', 'pause', 'seek', 'volume')
   * @param {*} payload - Parameter or payload for the action
  ...
  async execute(actionName, payload, context = {}) {
    ...
    if (this.logger?.scope) {
      this.logger.scope('action').log(`(AdapterDriver) Executing '${actionName}' via Adapter [${adapterName}]`, { payload, instanceId: instId });
    }

    // Standardize internal action naming for actions.execute
    let mappedAction = actionName;
    if (actionName === 'seekTo') mappedAction = 'seekto';
