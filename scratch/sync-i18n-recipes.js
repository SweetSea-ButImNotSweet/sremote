...
const commentReplacements = [
  // Ready2use install
  { regex: /\/\/\s*🚀\s*(?:Cài đặt|Installation):\s*npm\s+install\s+@sremote\/ready2use\s+@sremote\/wrapper/gi, tag: '// [cmt_install_ready2use]' },
  // Wrapper install
  { regex: /\/\/\s*(?:1\.\s*)?(?:Cài đặt SDK|Install SDK):\s*npm\s+install\s+@sremote\/wrapper/gi, tag: '// [cmt_install_wrapper]' },
  // Mount auto
  { regex: /\/\/\s*(?:Khởi tạo và gắn player tự động trong 1 dòng lệnh|Auto-initialize and mount player in a single line of code)/gi, tag: '// [cmt_mount_auto]' },
  // Control via client
  { regex: /\/\/\s*(?:Điều khiển qua SRemote Client|Control media playback via SRemote Client)/gi, tag: '// [cmt_control_via_client]' },
  // Listen realtime
  { regex: /\/\/\s*(?:Lắng nghe sự kiện phát thời gian thực|Listen for real-time playback events)/gi, tag: '// [cmt_listen_realtime]' },
  // Register adapter
  {
  ...
  // Sync events
  { regex: /\/\/\s*(?:Đồng bộ sự kiện phát từ player sang SRemote|Synchronize player playback events into SRemote)/gi, tag: '// [cmt_sync_events]' },
  // YouTube Adapter / Init
  {
    regex:
      /\/\/\s*(?:3\.\s*)?(?:Khởi tạo YouTube IFrame API và kết nối Adapter vào SRemote|Initialize (?:YT\.Player and bind with SRemote Adapter \(Vanilla JS\)|YouTube IFrame API and bind Adapter into SRemote))/gi,
  ...
  // Spotify adapter
  { regex: /\/\/\s*(?:Tạo SRemote Adapter kết nối Spotify Embed Controller|Create SRemote Adapter connecting Spotify Embed Controller)/gi, tag: '// [cmt_spotify_adapter]' },
  // Vimeo adapter
  { regex: /\/\/\s*(?:Tạo SRemote Adapter kết nối Vimeo Player SDK|Create SRemote Adapter connecting Vimeo Player SDK)/gi, tag: '// [cmt_vimeo_adapter]' },
  // SoundCloud adapter
  { regex: /\/\/\s*(?:Tạo SRemote Adapter kết nối SoundCloud Widget API|Create SRemote Adapter connecting SoundCloud Widget API)/gi, tag: '// [cmt_soundcloud_adapter]' },
  // Dailymotion adapter
  { regex: /\/\/\s*(?:Tạo SRemote Adapter kết nối Dailymotion Player SDK|Create SRemote Adapter connecting Dailymotion Player SDK)/gi, tag: '// [cmt_dailymotion_adapter]' },
  // Twitch adapter
  {
  ...
  // Mixcloud adapter
  { regex: /\/\/\s*(?:Tạo SRemote Adapter kết nối Mixcloud PlayerWidget|Create SRemote Adapter connecting Mixcloud PlayerWidget)/gi, tag: '// [cmt_mixcloud_adapter]' },
  // Native HTML5
  {
